...
long long unsigned paired=0;
int pids[100]={0};
int pid,stat;


void process(char **s,long long unsigned l);
/*long long unsigned skobki(char** s, long long unsigned len){
	for(long long unsigned i; i<len; i++){
		if(strlen(s[i])==2 && strchr("()") )
	}
}*/
...
void process(char **a,long long unsigned l) {
    ...
    int status,f=1,fd[2]={0},cp[2]={dup(0),dup(1)};
    while (a[i]) {
	if (t==0 && !strcmp(a[i],"cd")) {
	    if (f) {
		...
		while (wait(&status)!=-1);
		if (a[i]) {
		    if (!strcmp(a[i],"&&")) f=WIFEXITED(status);
		    if (!strcmp(a[i],"||")) f=WIFEXITED(status)? 0 : 1;
		    i++;
		}
